...
void imp_layermenu_render(SDL_Renderer *renderer, ImpCanvas *canvas, ImpLayerMenu *menu) {
    ...
    int butsize = menu->rect.w / 2;
    SDL_RenderCopy(renderer, menu->plus_texture, NULL, &(SDL_Rect){ menu->rect.x, menu->rect.y, butsize, butsize });
    SDL_RenderCopy(renderer, menu->minus_texture, NULL, &(SDL_Rect){ menu->rect.x + butsize, menu->rect.y, butsize, butsize });

    uc r, g, b, a;
    int ystart = menu->rect.y - butsize;
